...
class RimNumber {
    ...
    static Number parser(String symbol) throws Exception {
    ...
    static Number parser(String symbol, Perebor type) throws Exception {

        Number number = parser(symbol);
        if (number.getType() != type) {
            throw new Exception("Разные типы данных");
        }
    ...
    static String RomanNumber(int number) {
        ...
        return romanString.get(i) + RomanNumber(number - i);
    }
